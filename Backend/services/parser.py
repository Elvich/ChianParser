...
def generate_semantic_tags(apt: Apartment) -> List[str]:
    ...
    if apt.is_auction:
        tags.append("Аукцион")
    if apt.is_alternative:
        tags.append("Альтернатива")
    if apt.is_share:
        tags.append("Доля")
    if apt.is_apartment:
        tags.append("Апартаменты")
...
def update_views_snapshot(
    apt: Apartment, old_views_total: Optional[int], old_last_update: datetime
) -> None:
    """
    Фиксирует снэпшот просмотров для расчета суточной дельты, аналогично клиенту.
    """
    if (
        old_views_total is not None
        and apt.views_total is not None
        and apt.views_total > old_views_total
    ):
...
async def parse_detail_page(page, cian_id: str) -> dict:
    ...
    try:
        ...
        try:
            await page.get_by_text(
                re.compile("просмотр", re.IGNORECASE)
            ).first.wait_for(state="attached", timeout=5000)
            logger.info(
                f"📊 Блок статистики просмотров успешно загружен на странице {cian_id}"
            )
        except Exception as e:
            logger.warning(
                f"⚠️ Не дождались загрузки блока статистики (текст 'просмотр') для {cian_id}: {e}"
            )
    ...
    try:
        ...
        if json_str:
            ...
            if offer_data:
                ...
                price = (
                    bargain_terms.get("price")
                    or bargain_terms.get("priceRur")
                    or bargain_terms.get("priceTotal")
                    or bargain_terms.get("priceTotalRur")
                )
                if price:
                ...
                detail_data["description"] = offer.get("description") or offer.get(
                    "text"
                )
                ...
                if len(image_urls) < 3:
                    brute_urls = re.findall(
                        r"https://cdn-p\.cian\.site/[^\s\"']+\.jpg", json_str
                    )
                ...
                detail_data["max_floor"] = building.get("floorsCount") or building.get(
                    "floors"
                )
                detail_data["year_built"] = building.get("buildYear") or building.get(
                    "year"
                )
                detail_data["house_material"] = building.get(
                    "materialType"
                ) or building.get("material")
                ...
                if lifts:
                    try:
                        lifts_int = int(lifts)
                        detail_data["elevator"] = (
                            f"{lifts_int} шт." if lifts_int > 0 else None
                        )
                ...
                detail_data["balcony"] = offer.get("balconyType") or offer.get(
                    "balcony"
                )
                ...
                if address_comps:
                    ...
                    detail_data["address"] = geo.get("displayAddress") or geo.get(
                        "userInputAddress"
                    )
                ...
                if undergrounds and isinstance(undergrounds, list):
                    ...
                    if isinstance(metro, dict):
                        detail_data["metro_name"] = metro.get("name") or metro.get(
                            "title"
                        )
                        detail_data["metro_distance_minutes"] = (
                            metro.get("travelTime")
                            or metro.get("time")
                            or metro.get("distance")
                        )
                        detail_data["metro_transport_type"] = metro.get(
                            "travelType"
                        ) or metro.get("transportType")

                # Статистика
                stats = offer.get("stats") or {}
                detail_data["views_total"] = (
                    stats.get("total")
                    or stats.get("totalViews")
                    or stats.get("allViews")
                )
                detail_data["views_today"] = (
                    stats.get("daily")
                    or stats.get("dailyViews")
                    or stats.get("today")
                    or stats.get("todayViews")
                    or stats.get("viewsToday")
                    or stats.get("dayViews")
                )

                # Разбор строки просмотров, если нет в полях
                if not detail_data.get("views_total") or not detail_data.get(
                    "views_today"
                ):
                    formatted_str = stats.get("totalViewsFormattedString") or offer.get(
                ...
                # Особые условия
                sale_type = str(
                    bargain_terms.get("saleType") or offer.get("saleType") or ""
                ).lower()
                desc_lower = (detail_data.get("description") or "").lower()

                detail_data["is_auction"] = bool(
                    offer.get("isAuction")
                    or "auction" in sale_type
                    or "аукцион" in desc_lower
                    or (
                        detail_data.get("title")
                        and "аукцион" in detail_data["title"].lower()
                    )
                )
                ...
                detail_data["is_studio"] = (
                    "studio" in flat_type
                    or "studio" in category
                    or "студия" in desc_lower
                )
                detail_data["is_apartment"] = (
                    "apartment" in category or "апартамент" in desc_lower
                )
                ...
                detail_data["is_deposit_paid"] = any(
                    p in desc_lower for p in deposit_phrases
                )

                # Продвижение
                placement = str(
                    offer.get("placementType") or offer.get("promotionType") or ""
                ).lower()
                ...
                if pub_date_str:
                    try:
                        detail_data["published_date"] = datetime.fromisoformat(
                    ...
                    try:
                        detail_data["published_date"] = datetime.utcfromtimestamp(
                            float(offer["addedTimestamp"])
                        )
                    except Exception:
                        pass
                ...
                detail_data["seller_name"] = (
                    seller.get("name")
                    or seller.get("alias")
                    or seller.get("companyName")
                )
                detail_data["seller_type"] = seller.get("type") or seller.get(
                    "category"
                )
    ...
    try:
        ...
        for selector in selectors["metroSelectors"]:
            loc = page.locator(selector)
            if await loc.count() > 0:
                detail_data["metro_name"] = (
                    (await loc.first.inner_text()).strip().split("\n")[0]
                )
                break
            ...
            if count > 0:
                for i in range(count):
                    ...
                    if title and value:
                        if "общая" in title:
                            try:
                                detail_data["space"] = float(
                                    re.sub(r"[^\d\.]", "", value.replace(",", "."))
                                )
                            except Exception:
                                pass
                        elif "жилая" in title:
                            try:
                                detail_data["living_space"] = float(
                                    re.sub(r"[^\d\.]", "", value.replace(",", "."))
                                )
                            except Exception:
                                pass
                            ...
                            try:
                                detail_data["kitchen_space"] = float(
                                    re.sub(r"[^\d\.]", "", value.replace(",", "."))
                                )
                            except Exception:
                                pass
                            ...
                            if m:
                                ...
                                if m:
                                    try:
                                        detail_data["floor"] = int(m.group(0))
                                    except Exception:
                                        pass
                        elif "построен" in title or "год постройки" in title:
                            try:
                                detail_data["year_built"] = int(
                                    re.search(r"\d+", value).group(0)
                                )
                            except Exception:
                                pass
                        elif "высота потолков" in title:
                            try:
                                detail_data["ceiling_height"] = float(
                                    re.sub(r"[^\d\.]", "", value.replace(",", "."))
                                )
                            except Exception:
                                pass
                            ...
                            detail_data["furniture"] = (
                                "да" in value.lower() or "есть" in value.lower()
                            )
        ...
        if detail_data.get("description"):
            ...
            if "аукцион" in desc_lower:
                detail_data["is_auction"] = True
            if "студия" in desc_lower:
                detail_data["is_studio"] = True
            if "апартамент" in desc_lower:
                detail_data["is_apartment"] = True
            if "доля" in desc_lower:
                detail_data["is_share"] = True

        # DOM Fallback для просмотров
        if not detail_data.get("views_total") or not detail_data.get("views_today"):
            logger.info(
                f"🔄 Запуск DOM Fallback для извлечения просмотров (cian_id: {cian_id})..."
            )
                ...
                for i in range(count):
                    try:
                        ...
                        if text:
                            ...
                            if views_found:
                                logger.info(
                                    f"📊 Просмотры найдены через селектор {selector}: {text} -> total: {total}, today: {today}"
                                )
                                break
                    except Exception as ex:
                        logger.warning(
                            f"Ошибка при разборе текста из селектора {selector}: {ex}"
                        )
                if views_found:
                    break
                ...
                try:
                    ...
                    for line in lines:
                        if "просмотр" in line.lower() or "за сегодня" in line.lower():
                            ...
                            if views_found:
                                logger.info(
                                    f"📊 Просмотры найдены через BeautifulSoup get_text() построчно: {line} -> total: {total}, today: {today}"
                                )
                                break
                    ...
                    if not detail_data.get("views_total") or not detail_data.get(
                        ...
                        match = re.search(
                            r".{0,50}просмотр.{0,50}", single_line_text, re.IGNORECASE
                        )
                        if match:
                            ...
                            if today is not None:
                                detail_data["views_today"] = today
                            logger.info(
                                f"📊 Просмотры найдены в текстовом сниппете: {snippet} -> total: {total}, today: {today}"
                            )
                except Exception as ex:
                    logger.warning(
                        f"⚠️ Ошибка при извлечении просмотров через BeautifulSoup: {ex}"
                    )
    ...
    if not detail_data.get("published_date"):
        try:
            ...
            if pub_match and len(pub_match.group(1)) >= 10:
                detail_data["published_date"] = datetime.fromisoformat(
                    pub_match.group(1).replace("Z", "+00:00")
                )
            else:
            ...
            logger.warning(
                f"⚠️ Ошибка при поиске даты публикации в HTML для {cian_id}: {e}"
            )
    ...
    if "published_date" in detail_data and detail_data["published_date"]:
        try:
            ...
            logger.info(
                f"📊 Запрос детальной статистики просмотров для {cian_id}: {stat_url}"
            )

            views_history = await page.evaluate(
                """async (url) => {
                try {
            ...
            logger.warning(
                f"⚠️ Не удалось загрузить детальную статистику просмотров для {cian_id} через evaluate: {ev}"
            )
...
async def run_stealth_parser() -> None:
    ...
    async with async_session_maker() as session:
        config_statement = select(ScoringConfiguration).where(
    ...
    if settings.PROXY_URL:
        try:
            ...
            logger.info(
                f"Используется мобильный прокси: {parsed.hostname}:{parsed.port}"
            )
        except Exception as e:
    ...
    try:
        ...
        max_pages = (
            2  # Ограничиваемся 2 страницами на каждый поисковый URL для безопасности
        )
        ...
        logger.info(
            f"Сбор карточек завершен. Всего найдено уникальных: {len({c['cian_id'] for c in all_parsed_cards})}"
        )
        ...
        async with async_session_maker() as session:
            for card_data in all_parsed_cards:
                ...
                if (
                    needs_detail_parse
                    and detailed_scrapes_count >= max_detailed_scrapes
                ):
                    logger.info(
                        f"Достигнут лимит детальных парсингов ({max_detailed_scrapes}). Пропускаем глубокий парсинг для {cian_id}."
                    )
                    needs_detail_parse = False
                ...
                price_sqm = (
                    merged_data["price"] / merged_data["space"]
                    if merged_data["space"] > 0
                    else 0.0
                )
                ...
                if not apt:
                    ...
                    apt = Apartment(
                        cian_id=cian_id,
                        title=merged_data.get(
                            "title",
                            f"{merged_data['rooms']}-комн. квартира, {merged_data['space']} м²",
                        ),
                        ...
                        metro_name=merged_data.get("metro_name")
                        or merged_data.get("features", {}).get("metro_name"),
                        metro_distance_minutes=merged_data.get("metro_distance_minutes")
                        or merged_data.get("features", {}).get(
                    ...
                    if merged_data.get("is_detailed_parsed"):
                        ...
                        apt.living_space = merged_data.get(
                            "living_space", apt.living_space
                        )
                        apt.kitchen_space = merged_data.get(
                            "kitchen_space", apt.kitchen_space
                        )
                        apt.house_material = merged_data.get(
                            "house_material", apt.house_material
                        )
                        apt.year_built = merged_data.get("year_built", apt.year_built)
                        apt.ceiling_height = merged_data.get(
                            "ceiling_height", apt.ceiling_height
                        )
                        apt.bathroom_type = merged_data.get(
                            "bathroom_type", apt.bathroom_type
                        )
                        apt.balcony = merged_data.get("balcony", apt.balcony)
                        apt.window_view = merged_data.get(
                            "window_view", apt.window_view
                        )
                        ...
                        apt.metro_distance_minutes = merged_data.get(
                            "metro_distance_minutes", apt.metro_distance_minutes
                        )
                        apt.metro_transport_type = merged_data.get(
                            "metro_transport_type", apt.metro_transport_type
                        )
                        apt.parking = merged_data.get("parking", apt.parking)
                        apt.elevator = merged_data.get("elevator", apt.elevator)
                        apt.seller_name = merged_data.get(
                            "seller_name", apt.seller_name
                        )
                        apt.seller_type = merged_data.get(
                            "seller_type", apt.seller_type
                        )
                        apt.views_today = merged_data.get(
                            "views_today", apt.views_today
                        )
                        apt.views_total = merged_data.get(
                            "views_total", apt.views_total
                        )
                        apt.published_date = merged_data.get(
                            "published_date", apt.published_date
                        )
                        apt.views_history_json = merged_data.get(
                            "views_history_json", apt.views_history_json
                        )
                        apt.is_studio = merged_data.get("is_studio", apt.is_studio)
                        apt.is_apartment = merged_data.get(
                            "is_apartment", apt.is_apartment
                        )
                        apt.is_auction = merged_data.get("is_auction", apt.is_auction)
                        apt.is_deposit_paid = merged_data.get(
                            "is_deposit_paid", apt.is_deposit_paid
                        )
                        apt.is_alternative = merged_data.get(
                            "is_alternative", apt.is_alternative
                        )
                        apt.is_share = merged_data.get("is_share", apt.is_share)
                        apt.is_paid_promotion = merged_data.get(
                            "is_paid_promotion", apt.is_paid_promotion
                        )
                        apt.promotion_type = merged_data.get(
                            "promotion_type", apt.promotion_type
                        )
