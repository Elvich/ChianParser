...
async def test_api_key_protection(client: AsyncClient):
    ...
    response = await client.get("/api/v1/apartments/delta", headers={"X-API-Key": "wrong_key"})
    assert response.status_code == 401
...
async def test_scoring_config_and_delta(client: AsyncClient):
    ...
    response = await client.put("/api/v1/scoring/config", json=config_data, headers=headers)
    assert response.status_code == 200
