...
class Settings(BaseSettings):
    ...
    TELEGRAM_BOT_TOKEN: str = Field(
        default="", description="Токен Telegram-бота для отправки алертов"
    )
    TELEGRAM_CHAT_ID: str = Field(
        default="", description="ID чата Telegram для отправки алертов"
    )

    # Настройки S3 резервного копирования
    S3_BUCKET_NAME: str = Field(
        default="chianparser-backups", description="Имя бакета S3 для бэкапов"
    )
    S3_ENDPOINT_URL: str = Field(
        default="https://s3.amazonaws.com", description="Endpoint URL для S3"
    )
    AWS_ACCESS_KEY_ID: str = Field(default="", description="Access Key ID для S3")
    AWS_SECRET_ACCESS_KEY: str = Field(
        default="", description="Secret Access Key для S3"
    )
